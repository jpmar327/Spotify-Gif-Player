...
// Fetches the user's Liked Songs (saved tracks). Spotify's API caps this
// endpoint at 50 tracks per page — we only need one page's worth to start
// a play queue, so a single request is sufficient.
export async function getSavedTrackUris(accessToken: string): Promise<string[]> {
  const res = await loggedFetch(
  ...
  if (!res.ok) {
    throw Object.assign(new Error(`Saved tracks fetch failed: ${res.status}`), { status: res.status });
  }
  const data = (await res.json()) as { items: Array<{ track: { uri: string } }> };
  return (data.items ?? []).map((item) => item.track.uri);
}

// Starts playback with an explicit list of track URIs (used for Liked Songs,
// since Liked Songs has no context_uri the way playlists/albums do).
async function playTrackUris(uris: string[], token: string): Promise<void> {
  ...
  if (!res.ok && res.status !== 204) {
    throw Object.assign(new Error(`Play URIs failed: ${res.status}`), { status: res.status });
  }
}

// Smart play: tries a normal resume first (works if there's an active,
// paused device/context). If that fails with 404 — meaning no active device
// or context exists — falls back to fetching Liked Songs and starting there.
export async function smartPlay(token: string): Promise<void> {
  try {
    await resumePlayback(token);
    return;
  } catch (err) {
    const status = (err as { status?: number })?.status;
    // 404 = no active device/context to resume. Any other error (e.g. 403
    // missing scope, network failure) should surface normally.
    if (status !== 404) throw err;
  }

  const uris = await getSavedTrackUris(token);
  if (uris.length === 0) {
    throw new Error('No saved tracks found in Liked Songs');
  }
  await playTrackUris(uris, token);
}
